...
import {
  ...
  getAsyncStorageState,
} from '../utils/aptos/account';
...
export default function useWalletState() {
  const [asyncStorageState, setAsyncStorageState] = useState<
    AsyncStorageState | Promise<AsyncStorageState | null>
  ...
  const updateWalletState = useCallback(
    async ({aptosAccountState}: UpdateWalletStateProps) => {
      try {
